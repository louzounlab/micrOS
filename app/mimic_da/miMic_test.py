...
def calculate_all_imgs_tag_corr(samba_output, folder, tag, start_i, eval="corr", sis='fdr_bh', correct_first=False,
                                mode="test", threshold_p=0.05, THRESHOLD_edge=0.5, shuffle=False, colorful=None, path=""):
    """
...
def apply_mimic(folder, tag, eval="man", sis="fdr_bh", correct_first=True, mode="test", save=True, tax=None,
                ...
                preprocess='True', processed=None, apply_samba=True, samba_output=None, path=""):
    """
    ...
    with warnings.catch_warnings():
        ...
        if mode == 'test':
            ...
            if t1 != 'noAnova':
                ...
                if save:
                    df_corrs.to_csv(f"{folder}/miMic/df_corrs.csv")
            ...
            if tax != 'nosignificant':
                ...
                plot_rp_sp_anova_p(results_to_plot, mixed, save=path)
                # Plot RSP(beta) vs beta
                calculate_rsp(num_s_df, path)
